...
public class DaoPerson implements IDao{
    ...
    //Ci permette di adattarci meglio alle diverse query, ma è molto più lento, siamo nell'ordine del O(N^2)
    //Facendo il tutto per una query specifica possiamo passare ad un O(N) sfurttando una mappa con <Id, Object>
    public Map<Integer, Map<String, String>> readAll(String gimmeQuery){
        Map<Integer, Map<String, String>> responseMap = new HashMap<>();
        database.openConnection();
        try {
            PreparedStatement preparedStatement = database.getConnectiion().prepareStatement(gimmeQuery);
            ResultSet result = preparedStatement.executeQuery();
            //Mappa per la gestione di ogni riga
            ...
            while (result.next()) {
                ...
                responseMap.put(result.getInt(1), row);
            ...
            System.out.println("Sono nel metodo readAll, qualcosa non funziona col DB");
        } catch (Exception e){
            System.out.println("Sono nel metodo readAll, qualcosa è andato storto ma non è nel preparedStatement");
        }
        database.closeConnection();
        return responseMap;
    }
